  ...
  group('PatternMatcher', () {
    group('exact matching', () {
      ...
      test('handles empty patterns', () {
        expect(PatternMatcher.shouldSkipPath('/any/path', []), false);
      });
    });
    ...
    group('wildcard matching', () {
      test('matches paths with trailing wildcards', () {
        ...
        expect(
          PatternMatcher.shouldSkipPath('/admin/settings/advanced', [
            '/admin/*',
          ]),
          true,
        );
      ...
      test('matches paths with full wildcards', () {
        ...
        expect(PatternMatcher.shouldSkipPath('/debug', ['*debug*']), true);
    ...
    group('multiple patterns', () {
      test('matches any pattern in the list', () {
        ...
        expect(PatternMatcher.shouldSkipPath('/admin/users', patterns), true);
        ...
        expect(PatternMatcher.shouldSkipPath('/temp', patterns), true);

        expect(PatternMatcher.shouldSkipPath('/public/users', patterns), false);
      });
    });

    group('edge cases', () {
      test('handles root path', () {
        expect(PatternMatcher.shouldSkipPath('/', ['/']), true);

        expect(PatternMatcher.shouldSkipPath('/', ['/*']), true);
      });

      test('handles paths without leading slash', () {
        expect(PatternMatcher.shouldSkipPath('admin/users', ['admin/*']), true);
      });
      ...
      test('handles complex paths', () {
        expect(
          PatternMatcher.shouldSkipPath('/api/v1/users/123/settings', [
            '/api/*/users/*/settings',
          ]),
          true,
        );
        ...
        expect(
          PatternMatcher.shouldSkipPath('/deeply/nested/path/structure/here', [
            '/deeply/nested/*',
          ]),
          true,
        );
    ...
    group('real-world patterns', () {
      test('common skip patterns', () {
        ...
        // Should skip
        expect(
          PatternMatcher.shouldSkipPath('/debug/logs', commonPatterns),
          true,
        );
        expect(
        ...
        expect(
          PatternMatcher.shouldSkipPath('/app/health-check', commonPatterns),
          true,
        );
        expect(
          PatternMatcher.shouldSkipPath('/api/docs/swagger', commonPatterns),
          true,
        );
        expect(
          PatternMatcher.shouldSkipPath('/project/_dev/test', commonPatterns),
          true,
        );
        ...
        expect(
          PatternMatcher.shouldSkipPath('/admin/users', commonPatterns),
          false,
        );
        expect(
          PatternMatcher.shouldSkipPath('/api/users', commonPatterns),
          false,
        );
        expect(
          PatternMatcher.shouldSkipPath('/dashboard', commonPatterns),
          false,
        );
      });
    });
